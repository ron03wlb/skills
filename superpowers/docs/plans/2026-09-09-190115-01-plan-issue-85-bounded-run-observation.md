...
### Outcome 2: One durable fault-recovery budget
- Work: Extend the existing driver checkpoint and Run-owned store/task adapter seams to retain allowlisted fault identity, consumed rounds, fixed 5/15/30-second schedule, owner/request references, and settlement state. Make every read-only adapter retry consume the same persisted unresolved-fault budget so nested calls and re-entry cannot multiply it; healthy unchanged waits consume no rounds.
