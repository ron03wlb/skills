...
**Acceptance:** The exact Issue #20 Acceptance Criteria and parent constraints remain authoritative. New completion notes require an explicit empty list or exact path, requirement source, and purpose entries; declared paths must be owned by the Issue contribution and behaviorally eligible, and the declaration never becomes review, coverage, verification, cleanliness, or ref-stability authority. Before the first prospective completion, one logical repository/tracker/Spec-scoped adoption record binds the exact target scope: payload-identical concurrent publications collapse at the earliest history position, every later completion in that scope must carry the field, and a completion that definitely predates adoption or belongs to an unadopted scope remains legacy; conflicting or ambiguous scope evidence stops. Before either tracker mutation, require the clean reviewed candidate and exact adoption scope; on write or read-back failure, preserve repository state and report unresolved tracker ambiguity without integration, closure, push, or reset.
