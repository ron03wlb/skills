...
**Acceptance:** The exact Issue #20 Acceptance Criteria and parent constraints remain authoritative. New completion notes require an explicit empty list or exact path, requirement source, and purpose entries; declared paths must be owned by the Issue contribution and behaviorally eligible, and the declaration never becomes review, coverage, verification, cleanliness, or ref-stability authority. Immediately before completion, one immutable repository/tracker-specific adoption record binds the exact clean reviewed candidate and execution attempt: its following completion must carry the field, while an older completion without a matching record remains legacy; missing or ambiguous attempt evidence stops. Before either tracker mutation, require the clean reviewed candidate and exact attempt identity; on write or read-back failure, preserve repository state and report unresolved tracker ambiguity without integration, closure, push, reset, or duplicate evidence.
...
- Risks/open questions: The field and adoption record must never supply candidate mapping, contribution coverage, or a bypass for ordinary material documentation. Wall-clock or skill-repository cutovers cannot classify staggered consumer repositories and local-file trackers, so consumers use one read-back attempt-scoped tracker record and preserve older unmarked completions as legacy.
