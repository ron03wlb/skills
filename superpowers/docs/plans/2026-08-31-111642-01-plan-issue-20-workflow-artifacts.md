...
**Acceptance:** The exact Issue #20 Acceptance Criteria and parent constraints remain authoritative. New completion notes require an explicit empty list or exact path, requirement source, and purpose entries; declared paths must be owned by the Issue contribution and behaviorally eligible, and the declaration never becomes review, coverage, verification, cleanliness, or ref-stability authority. Before the first prospective completion, one durable repository/tracker/Spec-scoped adoption record binds the exact target scope: every later completion in that scope must carry the field, while a completion that definitely predates adoption or belongs to an unadopted scope remains legacy; missing or ambiguous scope evidence stops. Before either tracker mutation, require the clean reviewed candidate and exact adoption scope; on write or read-back failure, preserve repository state and report unresolved tracker ambiguity without integration, closure, push, reset, or duplicate evidence.
...
- Risks/open questions: The field and adoption record must never supply candidate mapping, contribution coverage, or a bypass for ordinary material documentation. Wall-clock, skill-repository, or optional attempt cutovers cannot classify staggered consumer repositories and local-file trackers, so consumers use one preceding Spec-scoped tracker record and fail plausibly bound malformed evidence closed.
