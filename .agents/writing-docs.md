...
Four sections make a page worth reading: `What it does`, `When to reach for it`, `Common questions`, `It's working if`. The first two orient the reader; the last two are where the page stops summarising the skill and starts answering the reader's own situation. Each of the last two has a bar to clear below and may be omitted when it does not clear that bar, but treat a page that clears neither as unfinished, not as finished-and-short.
