...
The Git-common-dir provenance that binds one producing workflow, Spec, target baseline, producer-owned authority identities, and an ordered **Producer checkpoint profile**. Reinvoking the same producer command may automatically resume an exact matching transaction from its first unsatisfied read-back stage without duplicating publication; any mismatch stops and preserves the evidence. Fresh current profiles persist only purpose-specific Planning Seal, publication, decomposition, ready-state, or handoff receipts and create no target operational-plan checkpoint. Frozen legacy and profile-v1 transactions retain their original plan, commit, and attestation stages without migration. A transaction is never the published contract or contribution authority itself; after `handoff.completed` it remains a small immutable receipt that downstream consumers may read idempotently, without automatic cleanup or a separate consumed state.
...
The target-scoped rule that permits only one planning or delivery operation to write one target branch at a time. `to-spec` Planning Seal writes, frozen legacy/profile-v1 workflow-plan checkpoints, and `close-issue` integration share this writer boundary, while fresh tracker-only producer publication, Issue worktrees, and other targets remain concurrent.
...
**Run-ready handoff**:
The minimal deterministic proof from the immediate upstream producer that the selected Spec and target may enter a **DAG Run**. A Single-Issue Spec consumes `to-spec`'s `handoff.completed`. For a Multi-Issue Spec, current `to-tickets` first consumes the exact `to-spec` publication and handoff, then emits one final `handoff.completed` that binds those upstream identities, its operation receipt, and the valid **Decomposition publication record** identity and digest; a frozen profile-v1 handoff retains both historical contribution-record identities. Run consumes only the final immediate-upstream handoff. In either route the target must be clean and no matching checkpoint transaction may remain active. It proves start readiness only and never repeats producer generation, whole-commit eligibility, content, review, or aggregate coverage validation.
...
- A Single-Issue Run consumes only `to-spec`'s completed handoff; for Multi-Issue, current `to-tickets` consumes the exact `to-spec` publication and handoff and emits one final **Run-ready handoff** binding those identities, its operation receipt, and the decomposition identity and digest, so Run never coordinates two producer transactions
