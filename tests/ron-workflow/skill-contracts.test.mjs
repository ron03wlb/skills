...
test("Codex-native workflow coordinator is explicit personal only", () => {
  ...
  assert.match(operator, /Single-Issue.*`to-spec`.*Multi-Issue.*`to-tickets`.*`READY`.*`INCOMPLETE`.*exact `\/to-spec <Spec-ID>` or `\/to-tickets <Spec-ID>`.*`UNKNOWN`.*stable diagnosis/isu);
