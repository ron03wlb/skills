...
const source = readFileSync(new URL("../../skills/personal/run-issue-workflow/references/codex-host-driver.md", import.meta.url), "utf8")
  .split("```js\n")[1].split("\n```")[0];
const Driver = Object.getPrototypeOf(async function () {}).constructor;

async function drive({ exitWhilePending = false } = {}) {
...
test("documented driver drains terminal output after host exit without replaying a pending native request", async () => {
  const observed = await drive({ exitWhilePending: true });
  assert.equal(observed.calls, 1);
