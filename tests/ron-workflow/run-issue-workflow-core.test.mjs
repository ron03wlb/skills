...
test("installed route keeps concurrent Spec operations isolated through same-command recovery", () => {
  const { root, gitCommonDir } = createGitCommonDirFixture("installed-route-producers-");
  try {
    const store = createWorkflowControlStore({ gitCommonDir });
    const specLane = checkpointIdentityV2({
      specId: "81",
      operationId: "publication:lane-81",
      profileVersion: "v2",
    ...
    const decompositionLane = checkpointIdentityV2({
      specId: "82",
      producerCommand: "to-tickets",
      operationId: "decomposition:lane-82",
      profileVersion: "v2",
      ...
      bindings: {
        ...
        upstream: {
          handoffIdentity: "handoff:to-spec:82",
          publicationIdentity: "tracker-version:82",
          trackerIdentity: "issue:82",
        },
      },
    });

    const initialSpec = store.createCheckpoint(specLane);
    const initialDecomposition = store.createCheckpoint(decompositionLane);
    store.advanceCheckpoint({
      identity: specLane,
      stage: "planning_seal.read_back",
    ...
    store.advanceCheckpoint({
      identity: decompositionLane,
      stage: "decomposition.read_back",
      receipt: { decompositionIdentity: "decomposition:82", mapping: { "82/01": "821" } },
    });
    store.advanceCheckpoint({
      identity: decompositionLane,
      stage: "ready_state.read_back",
      receipt: { frontier: ["821"] },
    });
    const completedDecomposition = store.advanceCheckpoint({
      identity: decompositionLane,
      stage: "handoff.completed",
      receipt: { handoffIdentity: "handoff:to-tickets:82" },
    });

    const stoppedSpec = store.createCheckpoint(specLane);
    assert.equal(stoppedSpec.scopeKey, initialSpec.scopeKey);
    assert.equal(stoppedSpec.transactionId, initialSpec.transactionId);
    assert.equal(stoppedSpec.nextStage, "publication.read_back");
    assert.equal(completedDecomposition.state, "COMPLETED");
    assert.deepEqual(store.createCheckpoint(decompositionLane), completedDecomposition);

    store.advanceCheckpoint({
      identity: specLane,
      stage: "publication.read_back",
      receipt: { publicationIdentity: "tracker-version:81", trackerIdentity: "issue:81" },
    });
    const repairedSpec = store.advanceCheckpoint({
      identity: specLane,
      stage: "handoff.completed",
      receipt: { handoffIdentity: "handoff:to-spec:81" },
    });

    assert.equal(repairedSpec.state, "COMPLETED");
    assert.equal(repairedSpec.scopeKey, initialSpec.scopeKey, "same-command retry keeps the exact operation");
    assert.deepEqual(store.createCheckpoint(decompositionLane), completedDecomposition, "repair must not mutate another lane");
    assert.notEqual(initialSpec.scopeKey, initialDecomposition.scopeKey);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
