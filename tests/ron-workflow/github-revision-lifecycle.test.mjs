...
test("formal GitHub reconciliation carries an unchanged closed child and scopes pre-execution revision blockers", async () => {
  ...
  try {
    ...
    git("add", "candidate.md");
    git("commit", "-m", "completed child");
    ...
    const old = {
      ...
      approvedScopeHash: bodyDigest("old parent"),
      decompositionIdentity: null,
    };
    const current = {
      ...old,
      approvedScopeHash: bodyDigest("new parent"),
      planningSeal: candidate,
    };
    const dec = {
      ...
      target: "main",
      planningSeal: seal,
      approvedScopeHash: old.approvedScopeHash,
      decompositionMapping: { "1/01": "I_2", "1/02": "I_3" },
      childBodyDigests: {
        I_2: bodyDigest("unchanged child"),
        I_3: bodyDigest("old blocked child"),
      },
      blockerEdges: [{ blocker: "I_2", blocked: "I_3" }],
      readyFrontier: ["I_3"],
    };
    const complete = {
      ...
      candidate,
      planningSeal: seal,
      operationIdentity: operation(old.approvedScopeHash),
      standards: "clean",
      spec: "clean",
      worktreeState: "clean",
      workflowArtifacts: [],
    ...
    const adoption = {
      issueId: "I_2",
      publicationIdentity: "IC_old_pub",
      decompositionIdentity: "IC_old_dec",
      childBodyDigest: dec.childBodyDigests.I_2,
      completionIdentity: done.node_id,
      completionBodySha256: bodyDigest(done.body),
    };
    const nextDec = {
      ...structuredClone(dec),
      planningSeal: candidate,
      approvedScopeHash: current.approvedScopeHash,
      childBodyDigests: {
        ...dec.childBodyDigests,
        I_3: bodyDigest("revised blocked child"),
      },
      adoptedCompletions: [adoption],
    };
    const checkpointIdentity = bindProducerCheckpointOperationIdentity({
      ...
      baseline: candidate,
      bindings: {
        approvedScopeIdentity: current.approvedScopeHash,
        classification: "MULTI",
        planningSeal: candidate,
      },
    });
    const blocked = {
      ...
      target: "main",
      planningSeal: seal,
      reasonCode: "scope_revision_required",
      operationIdentity: deriveExecuteIssueOperationIdentity({
        repositoryId,
        specId: "I_1",
        issueId: "I_3",
        approvedPublicationIdentity: old.approvedScopeHash,
      }),
    };
    const fixtures = [
      null,
      {
        ...
        comments: [
          ...
          comment("IC_old_dec", dec),
          comment("IC_new_pub", {
            kind: "spec_publication",
            repositoryId,
            authority: current,
          }),
          comment("IC_new_dec", nextDec),
          comment("IC_hand", {
      ...
      },
      {
        ...
        number: 3,
        state: "open",
        body: "revised blocked child",
        comments: [comment("IC_block", blocked)],
      },
    ];
    const checkpoints = createWorkflowControlStore({
      gitCommonDir: join(root, ".git"),
    });
    const oldIdentity = bindProducerCheckpointOperationIdentity({
      repositoryId,
      specId: "I_1",
      producerCommand: "to-tickets",
      profileVersion: "v2",
      ...
      baseline: seal,
      bindings: {
        approvedScopeIdentity: old.approvedScopeHash,
        classification: "MULTI",
        planningSeal: seal,
      },
    });
    ...
    const decompositionReadBack = {
      decompositionIdentity: "IC_old_dec",
      decompositionDigest: bodyDigest(fixtures[1].comments[1].body),
    };
    const readyStateReadBack = { frontier: ["I_2"] };
    checkpoints.advanceCheckpoint({
      identity: oldIdentity,
      stage: "decomposition.read_back",
      receipt: decompositionReadBack,
    });
    checkpoints.advanceCheckpoint({
      identity: oldIdentity,
      stage: "ready_state.read_back",
      receipt: readyStateReadBack,
    });
    const oldHandoff = {
      kind: "producer_handoff",
      producerCommand: "to-tickets",
      ...old,
      upstreamPublicationIdentity: "IC_old_pub",
      ...decompositionReadBack,
      checkpointIdentity: oldIdentity,
      transactionIdentity: oldTx.transactionId,
      operationReceipt: {
        transactionIdentity: oldTx.transactionId,
        decompositionReadBack,
        readyStateReadBack,
    ...
    const commandRunner = (name, args, options) => {
      ...
      else if (path.includes("blocked_by"))
        response = issue.number === 3 ? [fixtures[2]] : [];
    ...
    const owner = createGitHubWorkflowSources({
      ...
      store: {
        listRunIds: () => (oldEvents.length ? ["old-run"] : []),
        readEvents: () => oldEvents,
      },
      tasks: { read: async () => ({ state: "RUNNING" }) },
      commandRunner,
    });
    const refresh = async (journal = []) =>
      owner.sources.reconciliation.read({
        tracker: await owner.sources.tracker.read({ specId: "1" }),
        journal,
        request: {},
      });
    const updateDec = () => {
      fixtures[1].comments[4] = comment("IC_new_dec", nextDec);
    };
    const expectRejected = async (pattern) =>
      assert.ok(
        (await refresh()).facts.contradictions.some((item) =>
          item.evidence.some((message) => pattern.test(message)),
        ),
      );
    ...
    checkpoints.advanceCheckpoint({
      ...
      receipt: { handoffIdentity: "IC_old_hand" },
    });
    ...
    assert.ok(
      commandCalls.some(
        ({ name, args }) =>
          name === "gh" &&
          args[1] === "repos/example/repo/issues/2/comments?per_page=100",
      ),
    );
    ...
    assert.deepEqual(
      (await refresh()).facts,
      result.facts,
      "read-only re-entry is stable",
    );
    fixtures[1].comments[2] = comment("IC_old_hand", {
      ...oldHandoff,
      transactionIdentity: "foreign",
    });
    await expectRejected(/Previous decomposition checkpoint/u);
    fixtures[1].comments[2] = comment("IC_old_hand", {
      ...oldHandoff,
    ...
    nextDec.adoptedCompletions = [];
    updateDec();
    await expectRejected(/outside current proven authority/u);
    nextDec.adoptedCompletions = [adoption, adoption];
    updateDec();
    await expectRejected(/duplicate completion adoption/u);
    nextDec.adoptedCompletions = [adoption];
    updateDec();
    fixtures[2].body += " changed";
    ...
    for (const field of [
      ...
      adoption[field] = "foreign";
      updateDec();
      await expectRejected(/Adopted/u);
      adoption[field] = value;
    }
    updateDec();
    fixtures[2].state = "open";
    await expectRejected(/latest closed previous/u);
    fixtures[2].state = "closed";
    complete.candidate = git(
      "commit-tree",
      git("rev-parse", "HEAD^{tree}"),
      "-p",
      seal,
      "-m",
      "unintegrated candidate",
    );
    done.body = renderWorkflowRecord(complete);
    adoption.completionBodySha256 = bodyDigest(done.body);
    updateDec();
    ...
    await expectRejected(/Previous revision has a Run Grant/u);
    oldEvents = [];
    const active = await refresh([
      {
        type: "dispatch.recorded",
        issueId: "I_2",
        taskRef: { threadId: "live", hostId: "local" },
      },
    ]);
    assert.match(active.facts.contradictions[0].evidence[0], /live ownership/u);
    fixtures[3].comments[0] = comment("IC_block", {
      ...blocked,
      worktree: "old-lane",
    });
    await expectRejected(/unresolved lane/u);
    fixtures[3].comments[0] = comment("IC_block", {
      ...blocked,
      operationIdentity: { ...blocked.operationIdentity, key: "invalid" },
    });
    await expectRejected(/malformed/u);
    fixtures[3].comments[0] = comment("IC_block", {
      ...blocked,
      operationIdentity: deriveExecuteIssueOperationIdentity({
        repositoryId,
        specId: "I_1",
        issueId: "I_3",
        approvedPublicationIdentity: "unpublished",
      }),
    });
    ...
    fixtures[3].comments[0] = comment("IC_block", {
      ...
      operationIdentity: deriveExecuteIssueOperationIdentity({
        ...
        specId: "I_1",
        issueId: "I_3",
        approvedPublicationIdentity: current.approvedScopeHash,
      }),
    });
    assert.equal(
      (await refresh()).facts.nodes[1].completionState,
      "BLOCKED",
      "current-operation blocker remains active",
    );
    assert.equal(
      git("status", "--porcelain"),
      "",
      "reader performs no Git writes",
    );
  } finally {
