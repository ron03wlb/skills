  ...
  try {
    ...
    assert.throws(
      () =>
        writer.append({
          ...event,
          workflowVersion: { ...workflowVersion, id: "c".repeat(64) },
        }),
      /preserve.*workflow version/u,
    );
    writer.append(event);
    assert.equal(store.readEvents(runIdentity.runId).length, 2);
    assert.deepEqual(
      store.readEvents(runIdentity.runId)[1].workflowVersion,
      workflowVersion,
    );
  } finally {
    writer.release();
...
test("an installed entry update preserves a running workflow's exact executable version", () => {
  ...
  writeFileSync(
    join(sourceRepository, "docs/agents/run-preparation.md"),
    "Shared planning preparation contract\n",
  );
  ...
  writeFileSync(
    ...
    "Shared stop diagnosis owner\n",
  );
  const commit = (version) => {
    copyHostAssets(sourceRepository);
    writeFileSync(
      join(sourceRepository, entry),
  ...
  try {
    ...
    for (const path of hostAssets)
      assert.deepEqual(
        readFileSync(
          join(first.root, "skills/personal/run-issue-workflow", path),
        ),
        execFileSync("git", [
          "-C",
          sourceRepository,
          "show",
          `${first.version.sourceCommit}:skills/personal/run-issue-workflow/${path}`,
        ]),
      );
    assert.equal(
    ...
    assert.equal(
      execFileSync(
        process.execPath,
        [join(skillDirectory, "scripts/installed-entry.mjs")],
        { encoding: "utf8" },
      ).trim(),
      "v1",
    );
    ...
    let failed = false;
    fs.symlinkSync = (...args) => {
      if (!failed && args[1] === skillDirectory) {
        failed = true;
        throw new Error("Simulated new-entry failure");
      }
    ...
    try {
      assert.throws(
        ...
        (error) => {
          ...
          assert.equal(
            JSON.parse(readFileSync(error.recovery.recoveryPath))
              .previousTarget,
            join(first.root, "skills/personal/run-issue-workflow"),
          );
          return /Simulated new-entry failure.*installation recovery/u.test(
            error.message,
          );
        },
      );
    ...
    assert.equal(
      selectWorkflowVersion({ cacheDirectory }).state,
      "UNAVAILABLE",
    );
    assert.equal(
      selectWorkflowVersion({ cacheDirectory, recordedVersion: first.version })
        .state,
      "AVAILABLE",
    );
    // An intervening unknown entry must survive the same-command recovery attempt.
    fs.unlinkSync(skillDirectory);
    fs.symlinkSync(
      sourceRepository,
      skillDirectory,
      process.platform === "win32" ? "junction" : "dir",
    ...
    assert.equal(
      readFileSync(join(second.root, "docs/agents/run-preparation.md"), "utf8"),
      "Shared planning preparation contract\n",
    );
    assert.equal(
      fs.existsSync(join(cacheDirectory, "installation-pending.json")),
      false,
    );
    ...
    const repeated = installWorkflow({
      sourceRepository,
      sourceCommit: secondCommit,
      cacheDirectory,
      skillDirectory,
    ...
    const scripts = join(
      second.root,
      "skills/personal/run-issue-workflow/scripts",
    );
    ...
    assert.equal(
      selectWorkflowVersion({ cacheDirectory }).state,
      "UNAVAILABLE",
      "unchanged bytes outside the package remain rejected",
    );
    fs.unlinkSync(scripts);
    fs.renameSync(scripts + ".preserved", scripts);
    const resumed = selectWorkflowVersion({
      cacheDirectory,
      recordedVersion: first.version,
    });
    assert.equal(resumed.state, "AVAILABLE");
    assert.equal(
      execFileSync(process.execPath, [join(resumed.root, entry)], {
        encoding: "utf8",
      }).trim(),
      "v1",
    );
    writeFileSync(join(resumed.root, entry), "console.log('tampered');\n");
    const unavailable = selectWorkflowVersion({
      cacheDirectory,
      recordedVersion: first.version,
    });
    ...
    writeFileSync(
      join(second.root, "docs/agents/run-preparation.md"),
      "tampered shared authority\n",
    );
    assert.equal(
      selectWorkflowVersion({ cacheDirectory, recordedVersion: second.version })
        .state,
      "UNAVAILABLE",
    );
  } finally {
...
test("the exact install command recovers repeated interruptions before the catalog or backup exists", () => {
  ...
  try {
    ...
    const earlyOptions = {
      sourceRepository,
      sourceCommit,
      cacheDirectory: join(root, "early-cache"),
      skillDirectory: join(root, "early-entry"),
    };
    fs.writeFileSync = (path, ...args) => {
      if (
        path.startsWith(
          join(earlyOptions.cacheDirectory, "installation-pending."),
        )
      ) {
    ...
    assert.throws(
      ...
      /Interrupted before durable intent/u,
    );
    ...
    installWorkflow(earlyOptions);
    assert.equal(
      selectWorkflowVersion({ cacheDirectory: earlyOptions.cacheDirectory })
        .state,
      "AVAILABLE",
    );
    const killedOptions = {
      sourceRepository,
      sourceCommit,
      cacheDirectory: join(root, "killed-cache"),
      skillDirectory: join(root, "killed-entry"),
    };
    ...
    assert.throws(
      () =>
        execFileSync(
          process.execPath,
          ["--input-type=module", "-e", killedScript],
          { stdio: "ignore" },
        ),
      (error) =>
    ...
    const killedPending = readFileSync(
      join(killedOptions.cacheDirectory, "installation-pending.json"),
      "utf8",
    );
    assert.throws(
      () => installWorkflow(killedOptions),
      (error) =>
        error.message.includes(killedLock) &&
        /Prove no active installer.*preserve and move.*same exact install/u.test(
          error.message,
        ),
    );
    assert.equal(
    ...
    assert.equal(
      selectWorkflowVersion({ cacheDirectory: killedOptions.cacheDirectory })
        .state,
      "AVAILABLE",
    );
    for (const previousEntry of [false, true]) {
      ...
      syncBuiltinESMExports();
      assert.throws(
        () => installWorkflow(options),
        /Interrupted initial installation/u,
      );
      ...
      if (previousEntry)
        assert.equal(fs.readlinkSync(recovered.backup), sourceRepository);
      assert.equal(
        fs.existsSync(join(cacheDirectory, "installation-pending.json")),
        false,
      );
...
test("installation preserves an unknown local skill before touching its contents", () => {
  ...
  try {
    ...
    writeFileSync(join(skillDirectory, "SKILL.md"), "user-owned\n");
    assert.throws(
      () =>
        installWorkflow({
      ...
      /Unknown installed skill/u,
    );
    assert.equal(
      readFileSync(join(skillDirectory, "SKILL.md"), "utf8"),
      "user-owned\n",
    );
    const cacheDirectory = join(root, "bad-catalog");
    mkdirSync(cacheDirectory);
    writeFileSync(
      join(cacheDirectory, "installation.json"),
      JSON.stringify({
