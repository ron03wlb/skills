...
2. Dispatch dependency-ready Issues without exceeding `max_parallel`; every Issue has one lane. `execute-issue` owns its dedicated Issue worktree: the Codex task creates it, then execution verifies and adopts that exact worktree. A recorded creation intent prevents duplicate tasks after a lost response. Retry, repair, and recovery continuations reserve their allowlisted Run/Issue/message identity before native delivery; accepted or unresolved original owners survive restart and omitted task history without another send. A planning-prepared prerequisite lane is verified and adopted as that same task/worktree, then receives its first execution message under the Run Grant.
   ...
   Native observation emits a versioned allowlisted `task.outcome` receipt into the existing Run journal. It binds Run, Issue, operation, request, task, producer/package revision, phase, disposition, evidence locators/digests, pending or accepted effect identities, failure fingerprint, progress instants, and explicit byte/history budgets. It never retains worker prose, credentials, or raw host cursors. Normal active and successful completion reconciliation uses compact status/settlement evidence and never reads full task history; exceptional, contradictory, lost-ack, close, repair, and recovery ownership may use at most four exact bounded history reads.
   Five minutes without a verified status/cursor revision for the same task and phase emits one attributed `NEEDS_ATTENTION` receipt and performs one bounded diagnostic history read. Heartbeats, elapsed time, another task, and a still-live call are not progress. This does not interrupt, kill, replace, or settle healthy native work. A verified revision resets the clock; re-entry reuses the journaled progress/escalation instead of starting another timer or owner.
