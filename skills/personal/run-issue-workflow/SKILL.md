...
Repository-owned `run-authority-adapters.mjs` binds owning-source authority; `run-workflow.mjs` is the single composition interface. After `READY` and Grant read-back, read [the coordinator lifecycle](references/coordinator-lifecycle.md) only when opening or controlling the panel, reconciling live state, dispatching or adopting an Issue lane, waiting for the target writer, or executing a reducer action.
