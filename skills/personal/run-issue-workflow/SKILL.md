...
- `close_issue`: require a valid `implementation_complete`, send the same Issue lane one evidence-bound `close-issue` follow-up under the unchanged Grant, and wait for it to settle. That real leaf alone acquires the repository close lease followed by the shared target mutation writer, performs closeout, and releases both after settlement. The coordinator acquires, releases, reclaims, and delegates neither lease.
- `close_parent`: after every exact child has node success, invoke one evidence-bound parent-only `close-issue` leaf in the coordinator task. That real leaf owns the same repository-then-target lease order and read-back contract; the coordinator re-reads parent state only after the leaf settles.
