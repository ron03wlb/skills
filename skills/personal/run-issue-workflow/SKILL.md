...
Healthy foreign target writer contention enters a bounded `WAITING_FOR_TARGET_WRITER` coordinator state after every currently legal Issue dispatch. Append an exact `target-writer-wait.started` journal event with the pre-wait Run, Grant, target, tracker, candidate, completion, worktree, and control-revision evidence, then append one exact `target-writer-wait.settled` journal event; the wait consumes no Issue execution slot and never releases, reclaims, or replaces the other owner. After release, reacquire tracker, target, candidate, completion, worktree, control revision, and Grant evidence and compare the exact snapshot before recording `RELEASED` or acquiring the writer. Changed or unavailable evidence records `EVIDENCE_CHANGED` and returns a Recoverable blocker instead of invoking closeout.
...
- `wait_target_writer`: observe the exact healthy owner for at most the reducer-provided bound. Writer absence first reacquires and compares the exact pre-wait evidence; only an unchanged comparison settles `RELEASED`, while changed or unavailable evidence settles `EVIDENCE_CHANGED`. Owner change, timeout, coordinator loss, or an orphaned prior wait settles its exact exceptional outcome without target mutation. A control-revision change settles `CONTROL_CHANGED` and abandons the stale action batch.
