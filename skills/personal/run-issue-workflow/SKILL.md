...
Every explicit invocation first reads normalized retention evidence, produces the auditable cleanup preview, and applies the eligible terminal-Run sweep before acquiring a Run writer. A `cleanupPreview: true` request performs the same proof and selection without deletion. Cleanup never runs because of Stop, closeout, or panel closure.
