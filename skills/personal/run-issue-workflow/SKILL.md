...
`/run-issue-workflow <Spec-ID>` selects that exact Spec and authorizes its necessary Codex Issue tasks and later execution/close messages. Pass this identity to the installed entry; its owning sources read the body, comments, classification, target, approved scope, Planning Seal and Decomposition before action. Inspect only the scope or diagnosis needed for the current action; full history stays available on demand. Reject missing, contradictory, stale, or inaccessible authority.
