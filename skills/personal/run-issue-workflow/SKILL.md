...
- `INCOMPLETE` requires one exact consistent `to-spec` or `to-tickets` active or incomplete transaction whose producer, Spec, target, Planning Seal, classification, approved-scope identity, baseline, initially-clean state, plan path, generated-content identity, transaction identity, and first unsatisfied stage are all bound. A dirty target qualifies only when the adapter proves that exact producer owns the dirt; ordinary or unknown dirt is `UNKNOWN`. Return the exact `/<producer> <Spec-ID>` retry command, compact evidence, producer as next owner, and retry predicates before any Run mutation. Run never resumes the producer, commits its plan, attests, publishes, relabels, or deletes the receipt.
