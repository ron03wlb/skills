...
Before that authority or any mutation, Entry invokes the owning-source handoff adapter once with the already-read tracker and reconciliation snapshots, then reduces one immediate-upstream producer handoff. A Single-Issue Spec consumes only `to-spec`'s completed handoff; a Multi-Issue Spec consumes only `to-tickets`' completed composite handoff with both immutable v1 record identities and the exact Decomposition publication identity. `READY` continues only when the live selected Planning Seal also matches. `INCOMPLETE` returns the exact `/to-spec <Spec-ID>` or `/to-tickets <Spec-ID>` retry command, transaction identity, first unsatisfied stage, and producer-owned retry predicates only after exact transaction and any dirty-path ownership agree. `UNKNOWN` returns a stable diagnosis, exact observed checkpoint and handoff values, and recovery predicates for missing, dirty, malformed, contradictory, stale, legacy, or ambiguous evidence. Neither state applies cleanup, acquires a writer, records a Grant, opens the panel, acts on a task or leaf, or repairs a producer; the returned cleanup preview remains read-only.
