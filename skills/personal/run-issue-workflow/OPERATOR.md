...
An installation receipt is complete only when `readWorkflowInstallationEvidence` re-reads the selected package manifest, binds its source commit and manifest SHA-256, proves every configured managed entry resolves to that exact package directory, and invokes the package-owned routing, identity and Windows-continuation qualification through that selected `installed-entry.mjs`. Caller-authored PASS text is not evidence. A missing entry, foreign target, changed manifest, candidate mismatch, incomplete stage set or result from another package stays visible and fails verification. The runtime result separately reports the package version, package root and manifest digest actually selected by `installed-entry.mjs`; a compatible retained Run keeps its original Grant while exposing those current runtime bytes. Returned or emitted failures after selection retain that runtime evidence, while failures before package selection do not invent it.
...
Each Issue operation also exposes its cumulative six-hour execution budget. Implementation, retry, implementation repair, conflict repair, and their owned verification/review count across all retries, replacement tasks, restarts, re-entry, and material waves. Verified healthy dependency and writer waits are excluded, including healthy contention beyond twelve hours. At six hours the status reports `execution_timeout`; no new execution, repair, retry, or close request is scheduled for that Issue. The original task may still settle and its late result remains evidence, but the coordinator does not force-kill it or claim cancellation.

Unknown owner evidence, coordinator loss, or changed immutable authority returns a Recoverable blocker naming the owning source, exact evidence, smallest human action, preserved stages, and the same `/run-issue-workflow` command to retry. An exact owner whose health read is `UNKNOWN` first consumes the durable shared 5/15/30-second read-only observation policy; restored exact-generation health resumes waiting, while exhaustion reports `OWNER_HEALTH_UNKNOWN`. `UNKNOWN` is not inactive ownership. Do not steal, release, reclaim, cancel, or delegate either lease; do not close from the pre-wait snapshot or use elapsed time as reclaim proof. Every Issue leaf request carries current target state and exact HEAD plus exact Issue close authority evidence; a parent leaf request also carries exact parent tracker state and identity and every child's evidence. The real `close-issue` leaf alone acquires the repository close lease and then the target mutation writer.

The logical close-request identity excludes mutable Run progress and `controlRevision`, so a same-Run Pause or Resume does not make an already accepted close request look new. The coordinator still re-reads the current control revision before every legal action and abandons stale actions after Pause or Stop. A correctly attributed `HOST_CLEANUP_BLOCKED` result with an allowed cleanup reason routes to the close owner's existing `recoverPendingHostCleanup`; it does not create another close request or transfer cleanup ownership to the coordinator.
...
Issue #74's regression evidence is `tests/ron-workflow/installed-entry.test.mjs`, with real temporary Git repositories, installed package verification, checkpoints, Run journals and reconciliation. GitHub CLI and native Codex responses are fixtures. The tests compare original Grant/journal prefixes and observe native calls; they do not replay the closed Specs 64–71 or establish a new live delivery population. Existing package and batch suites cover installation recovery, shared capacity and serialized closeout.
