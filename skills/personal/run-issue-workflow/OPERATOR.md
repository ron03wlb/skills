...
An installation receipt is complete only when `readWorkflowInstallationEvidence` re-reads the selected package manifest, binds its source commit and manifest SHA-256, proves every configured managed entry resolves to that exact package directory, and binds each named failed-stage result to the same package version. A missing entry, foreign target, changed manifest, candidate mismatch, duplicate stage result or result from another package stays visible and fails verification. The runtime result separately reports the package version, package root and manifest digest actually selected by `installed-entry.mjs`; a compatible retained Run keeps its original Grant while exposing those current runtime bytes.
