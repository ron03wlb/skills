...
One physical stdin write retains one original owner and is observed for the configured transport wait plus a bounded host-overhead allowance. A still-pending write remains in the allowlisted checkpoint and blocks every second writer; its late original outcome resumes the same lane. A pending native Promise likewise stays with its original request and produces a compact fault observation without replaying the native call. Heartbeat ticks, transport observations and status frames emit only semantic deltas. Native responses larger than 6,000 characters travel as ordered fragments under the original request ID. The bridge accepts only the complete matching count, length and identity, then acknowledges the reconstructed response. After the driver's forwarded checkpoint is durable it sends `settleRequests` for those exact IDs; the bridge keeps only the response digest required for duplicate/conflict detection and discards the accepted request payload. Responses above 16 MiB of characters retain their original outcome and require diagnosis instead of payload truncation. Tool payloads and raw malformed frames stay in stored state; summaries omit them.

## Coordinator task observation

Use native `wait_threads` with a timeout no longer than 60 seconds. Observe at most eight exact task references per call, rotate larger sets fairly, and reuse each returned cursor. An unchanged running event does not justify a full task-history read. Read bounded history only for completion, attention, an anomalous event, or reconciliation that cannot be resolved from the compact event. If event waiting is explicitly unsupported, fall back to signal-aware polling after 15, 30 and then at most 60 seconds; only a semantic state transition resets that interval. Control and deadline signals interrupt the fallback independently of task state.
