...
Use native `wait_threads` in 15-second observation slices, below the 60-second maximum. Observe at most eight exact task references per call, rotate larger sets fairly, and reuse each returned cursor. Check queued control and the caller's optional `executionDeadlineAt` before and after an event slice. An unchanged running event does not justify a full task-history read. Read bounded history only for completion, attention, an anomalous event, or reconciliation that cannot be resolved from the compact event. If event waiting is explicitly unsupported, fall back to signal-aware polling after 15, 30 and then at most 60 seconds. Its lightweight exact-task snapshot excludes outputs; only a material terminal/anomalous result requests bounded full history. Only a semantic state transition resets the interval. Control and deadline signals interrupt the fallback independently of task state.

One canonical fault identity owns at most three read-only recovery rounds at 5, 15 and 30 seconds. Persist the round before its call so adapter re-entry cannot multiply the budget, and preserve an exhausted record instead of creating a nested retry loop. Observation summaries report native-call count, returned byte count and full-history reads. Model round trips and token counts remain `unavailable` unless the host supplies measured values.
