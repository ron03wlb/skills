...
`repair_issue` uses the original dispatched task and journals its candidate-bound wave before sending the request. Native request read-back prevents duplicate delivery after interruption. The budget is ten across re-entry, and a new completion must prove the replacement candidate’s verification and clean independent review before another close request. A known same-scope conflict is not a new permission request.
