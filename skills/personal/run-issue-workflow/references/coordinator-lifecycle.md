...
- `wait_repository_close_lease` or `wait_target_writer`: observe the healthy owner for the reducer bound. On absence, reacquire and compare the pre-wait evidence before `RELEASED`; record `EVIDENCE_CHANGED`, owner change, timeout, coordinator loss, orphaned wait, or `CONTROL_CHANGED` without mutation.
