...
GitLab may remove one final LF from an Issue description. Publication read-back accepts that specific omission or exact body equality; it does not trim other whitespace or accept content changes. The requested canonical body, approved hashes, original mutation payload and operation identity remain unchanged. The observed version still hashes the actual native body. An already acknowledged write with only this difference resumes the same transaction and appends its missing receipts without another PUT.
