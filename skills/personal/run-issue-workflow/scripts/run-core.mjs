...
export function reduceRun(input) {
  ...
  if (activeCloseWait) {
    ...
    const independent = normalActions.filter(action => ["dispatch_issue", "remediate_environment", "repair_issue", "upgrade_issue"].includes(action.type) && !dependsOnWait(action.issueId));
