...
import { readRepairProgress, readRepairWaveCount } from "./recovery-evidence.mjs";
...
export function readModelRepairBaseline({ records, lifecycle, journal, issueId, operationId }) {
  const counts = [...lifecycle.map(item => readRepairWaveCount(item.record)),
    ...journal.filter(event => event.issueId === issueId && (event.type === "repair.recorded"
      || event.type === "recovery.intent" && event.phase === "REPAIR")).map(event => event.wave)];
  if (counts.some(count => !Number.isInteger(count) || count < 0 || count > 10)) throw new Error("Recorded cumulative repair count is missing or invalid");
  // Model-specific progress remains subject to the complete two-wave proof below;
  // it cannot prove its own baseline and thereby bypass missing prior wave receipts.
  return readRepairProgress({ records: records.filter(item => item.record.kind === "implementation_progress"),
    issueId, operationId, count: Math.max(0, ...counts) });
}
...
export async function validateRepairYield({ evidence, runIdentity, issueId, taskRef, operationIdentity,
  ...
  for (const item of executionProgress) {
    ...
    if (readRepairWaveCount(record) !== record.repairWaves) throw new Error("Model repair progress count differs from its required encoding");
