...
A fresh ordinary decomposition uses `to-tickets@v2`. Derive and bind one versioned operation identity receipt from repository, Spec, approved publication identity or hash, producer `to-tickets`, and stage `decomposition`; bind the same transaction to tracker identity, profile `v2`, target, observed baseline, consumed Planning Seal, Multi-Issue classification, approved-scope identity, upstream publication identity, and upstream handoff identity. Caller correlation never defines authority. Its ordered stages are `decomposition.read_back`, `ready_state.read_back`, and `handoff.completed`.
