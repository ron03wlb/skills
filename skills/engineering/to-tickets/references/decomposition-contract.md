...
Every canonical child body carries the same logical blocker edges in its `## Blocked by` section for `body` or `native` representation. Body parent, target, Planning Seal, executable contract, canonical body edges, native parent evidence, and native blocking relation evidence in `native` representation must all agree wherever the tracker supports them. Titles are ignored. Neither representation adds a `blocked` label or a `relates_to` link; `ready-for-agent` remains a projection of the published logical graph. In `body` representation there is no native blocking relation; `native` representation uses native blocking relation evidence.
