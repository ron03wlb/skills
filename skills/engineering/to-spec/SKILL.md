...
Resolve the tracker identity before transaction creation. In primary mode, derive the versioned reservation identity from only the canonical repository and immutable proposed-Spec identity, call `tracker.reserve`, read one draft tracker identity and version token back, and reuse that same reservation on retry. In revision mode, require `tracker.read` of the existing Spec and its version token. Bind the read-back tracker identity to every later Spec-bound transaction and publication operation; caller correlation values never define authority.
