...
| Concrete GitLab Decomposition producer | The installed coordinator's `scripts/gitlab-to-tickets-entry.mjs`, `scripts/gitlab-to-tickets-adapters.mjs`, shared GitLab producer files, consumer `docs/agents/gitlab-producer.json`, and `Blocking representation` in `docs/agents/issue-tracker.md`, only for configured GitLab trackers | Run the installed entry's `inspect <repository>` and report its owning-source evidence separately from interface presence. Missing executable files or project binding is `MISSING`; missing blocker representation or unreadable provider evidence is `UNKNOWN`. Never call `invoke` during diagnostics. The producer supports tracker-only `to-tickets@v2`, not automatic Run composition. |
| Target reader | The installed coordinator's `scripts/run-authority-adapters.mjs` and `scripts/pi-workflow-host.mjs` | Repository-owned target and reconciliation readers are required by the delivery host; no target ref is changed. |
...
| Installed host entry and retained version | The separately installed coordinator's `scripts/installed-entry.mjs`, `workflows/deliver-tracker-spec/spec.json`, `scripts/pi-workflow-host.mjs`, and immutable package manifest | Resolve the source commit and verify package content; validate the delivery host bundle without starting a Run. A source path, symlink or fixture alone is not live delivery proof. |
| Run composition | The separately installed `run-issue-workflow` skill, `workflows/deliver-tracker-spec/spec.json`, and `scripts/pi-workflow-host.mjs` | The personal coordinator remains outside the promoted plugin while exposing one pi-workflow delivery host entrypoint. |
...
- for a missing GitLab Spec producer binding, name the installed coordinator's `references/gitlab-producer-adapters.md` and its separately authorized `configure` entry; for a missing Decomposition producer, name `references/gitlab-to-tickets-adapters.md` and distinguish a package update from the explicit consumer `Blocking representation` repair; re-running setup cannot create either adapter;
