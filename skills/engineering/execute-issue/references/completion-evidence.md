...
- when and only when the candidate equals the execution baseline under the validated documentation-only Seal exception, `reviewBasis` with `kind: planning_seal_documentation:v1`, exact `reviewBaseline` (the Seal's sole parent), exact `reviewCandidate` (the Planning Seal and final candidate), and the non-empty exact declared documentation-path list. The read-back Issue declaration and `reviewBaseline...reviewCandidate` diff must agree exactly; otherwise no completion is valid;
