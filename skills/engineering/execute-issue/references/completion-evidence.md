...
Read this reference only after final verification passes, Standards and Spec are clean, the Issue worktree is clean, and `HEAD` is the reviewed candidate. This file solely owns compatibility-adoption publication and the `implementation_complete` payload. It grants no integration, close, push, deployment, or external-execution authority.
