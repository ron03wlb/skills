...
Before the first prospective completion under an exact repository, tracker, parent or linked Spec, and Issue target branch, inspect that Spec's ordered history, including local-file tracker history, for one durable `workflow_artifacts_contract_adopted:v1` record. If none exists, append one binding those exact four identities and read it back once; otherwise reuse the one exact read-back record. It must precede every prospective completion in that scope. Duplicate, malformed, mismatched, or unreadable records plausibly bound to the same repository, tracker, and Spec stop without writing completion; write or read-back uncertainty reports unresolved tracker ambiguity and never creates a second record. The adoption makes this and all later completions in that exact scope prospective, so `workflowArtifacts` is required. A completion that definitely precedes adoption, or belongs to a scope with no adoption record, remains legacy; never add adoption retroactively to reclassify it. The record is compatibility evidence only and grants no implementation, review, coverage, verification, close, push, or deployment authority.
