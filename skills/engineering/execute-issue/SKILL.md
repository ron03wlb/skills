...
For coordinator entry, require one read-back DAG Run Grant that binds the exact linked Spec, Issue target branch, published Single-Issue or Multi-Issue classification, and approved scope. A Multi-Issue Grant must also bind the exact read-back **Decomposition publication record** whose mapping contains this Issue and whose published blockers make it dependency-ready. A missing, stale, or mismatched Grant stops before worktree creation or any mutation. The Grant adds no authority to execute a Manual prerequisite, change scope, repair ambiguity, integrate, close, push, or deploy.
