...
For a fresh or retried lane, read [`references/operation-identity.md`](references/operation-identity.md). Bind its deterministic operation identity before implementation and keep `execute-issue` as the implementation receipt owner; caller task or Run correlation never grants authority.
