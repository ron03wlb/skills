...
Write and read back exactly one completion note, then stop. Execution never integrates, removes a worktree, closes the Issue, pushes, or deploys. Execution never integrates or closes; the human or a coordinator holding the same valid DAG Run Grant separately invokes `/close-issue`.
