...
Run affected verification after every slice or repair. Reuse unchanged successful candidate-bound results through `scripts/verification-cache.mjs`; explicitly inventory configuration, environment (including relevant runtime and permission profile), and necessary external input versions, with a fresh owning-source reader on every lookup. A new integration combination needs its own result, even if each constituent candidate passed. Before review, run required focused checks, configured typechecking, and repository full suite; record exact commands and results. Late prerequisite discovery follows its reference before prerequisite-dependent verification.
