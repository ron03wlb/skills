...
Before the first prospective completion under an exact repository, tracker, parent or linked Spec, and Issue target branch, inspect that Spec's ordered history, including local-file tracker history, for one logical `workflow_artifacts_contract_adopted:v1` record. If none exists, append one durable record binding those exact four identities and read the ordered history back once; otherwise reuse the existing logical record. It must precede every prospective completion in that scope. Concurrent first completions may publish multiple payload-identical physical records; collapse them idempotently into one logical adoption at the earliest history position and never append another after any exact payload is visible. Malformed, mismatched, unreadable, or payload-conflicting records plausibly bound to the same repository, tracker, and Spec stop without writing completion; write or read-back uncertainty reports unresolved tracker ambiguity. The adoption makes this and all later completions in that exact scope prospective, so `workflowArtifacts` is required. A completion that definitely precedes adoption, or belongs to a scope with no adoption record, remains legacy; never add adoption retroactively to reclassify it. The record is compatibility evidence only and grants no implementation, review, coverage, verification, close, push, or deployment authority.
