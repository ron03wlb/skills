...
The `workflowArtifacts` cutover commit is `b1fcf9930056b1f1b907e380cd9015bd6162899d`. Every candidate whose Git ancestry contains that cutover commit, including the cutover candidate itself, is prospective and the field is required. Only a candidate proved to be an ancestor of that cutover is legacy; divergent or unreadable ancestry is not legacy. A producer or completion note cannot self-declare legacy status or replace this candidate-ancestry boundary with a timestamp or schema marker.
