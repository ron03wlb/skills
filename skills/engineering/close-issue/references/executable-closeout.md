...
Return verification identity and failed command/result at their existing owner. Coordinator emits only the exact compact `Workflow close result: <JSON>` schema: `issue-close-result:v1`, `INTEGRATION_FAILED|INTEGRATION_UNKNOWN`, exact `runId`, `issueId`, accepted `requestIdentity`, candidate, and `integrationVerification: {state,issueId,candidate,targetHead,identity}`. Automatic cleanup requires persisted PASS plus `externalInputs: { kind: "none" }`; external-input checks stay with the close owner. Obligations survive re-entry. Extra, malformed, or unverified integration fields become `UNCLASSIFIED` diagnosis and are never persisted as the close result. If cleanup later blocks, consume the owner-local [host-cleanup result contract](host-cleanup.md#host-cleanup-recovery) without restating its schema here.
...
After successful tracker read-back and while both leases remain current, call the lease handle's `markCompleted()` once. For a coordinator request, return exactly one native final-answer line `Workflow close result: <JSON>` with `schema: issue-close-result:v1`, `state: CLOSED`, exact `runId`, `issueId`, accepted `requestIdentity`, candidate, and the returned `deliveryProgress` containing distinct `repositoryCloseAcquiredAt`, `targetWriterAcquiredAt`, and `closeCompletedAt`. These owner timestamps are progression evidence; they do not replace fresh Git, tracker, worktree, or lease preconditions.

For transient failures, probe after 5, 15, and 30 seconds; retry skips completed actions and resumes the next one. Unknown ownership or exhausted probes preserves the lane. Target movement during partial close requires verification of the new combination; prior PASS preserves completed cleanup. Never roll back a successful merge.
