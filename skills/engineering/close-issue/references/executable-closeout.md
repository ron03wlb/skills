...
Return verification identity and failed command/result at their existing owner. Coordinator emits only the exact compact `Workflow close result: <JSON>` schema: `issue-close-result:v1`, `INTEGRATION_FAILED|INTEGRATION_UNKNOWN`, exact `runId`, `issueId`, accepted `requestIdentity`, candidate, and `integrationVerification: {state,issueId,candidate,targetHead,identity}`. Later `HOST_CLEANUP_BLOCKED` retains the PASS identity and only exact `{command,configFiles,environment,externalInputs}` descriptors. Automatic cleanup requires persisted PASS plus `externalInputs: { kind: "none" }`; external-input checks stay with the close owner. Obligations survive re-entry. Extra, malformed, or nested raw producer fields become `UNCLASSIFIED` diagnosis and are never persisted as the close result.
