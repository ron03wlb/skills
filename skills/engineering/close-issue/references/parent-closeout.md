...
Only then close and read back the parent, returning the coordinator [close result line](close-result-lines.md) for a coordinator-hosted close. Final-child closure never closes its parent implicitly; parent closure never claims `push_ready` or runs aggregate verification.
