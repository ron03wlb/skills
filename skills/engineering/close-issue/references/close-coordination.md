...
Within the same Git common directory, closeouts for different targets share one repository close lease; different repositories remain concurrent. The target mutation writer serializes planning producers and closeout on the exact target; Issue execution, planning on unrelated targets, and Issue worktrees never acquire the repository close lease.
