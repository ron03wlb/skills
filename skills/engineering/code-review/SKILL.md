...
- **Planning-Seal documentation candidate** — only when `execute-issue` supplies a published documentation-only Seal declaration and its validated `reviewBasis`. Resolve its sole-parent review baseline `P` and Planning Seal candidate `S`, require `S` to be the recorded execution baseline and candidate, and capture the non-empty `git diff P...S`. The diff must contain exactly the declaration's documentation paths. This form belongs only to that formal workflow; a requester cannot select it by calling any change documentation-only.

For committed work, note `git log <fixed-point>..<candidate-sha> --oneline`; for WIP, record current `HEAD` and the observed working-tree inputs; for the Planning-Seal form, record `P`, `S`, the declared paths, and the separately retained execution baseline `S`. Confirm the fixed point resolves with `git rev-parse <fixed-point>`. The candidate is non-empty when its tracked diff or at least one in-scope untracked path contains reviewable changes; the Planning-Seal form is non-empty only when its exact `P...S` diff is non-empty. A bad ref or empty candidate should fail here — not inside two parallel sub-agents.
