...
Load "grilling" as a named model-invoked skill before resolving a new choice: use the generic Skill tool when available, otherwise the host-supported mechanism to read and follow its required `SKILL.md`. Preserve the selected source and invocation restrictions, and reuse an already loaded matching owner and settled scope. Apply its decision boundaries and any supplied decision aperture. Record inherited decisions and evidence-backed reversible choices outside the human frontier directly, identifying their source, basis, practical reversal, and verification assumption. Reuse accepted decisions; bring new reserved choices, including table DDL and any migration cost, to the human with alternatives, trade-offs, reasons and a recommendation. Routine reversible queries and bindings follow the shared SQL boundary. Recording an ADR supplies neither missing SQL approval nor database execution authority.

### Challenge against the glossary

When a term conflicts with `CONTEXT.md`, identify the contradiction and check the governing requirement. Preserve the existing meaning when the evidence resolves it. For remaining ambiguity, apply the decision aperture: ask only when the choice is in the human frontier; otherwise select the project-consistent or established default and record its basis.

### Sharpen fuzzy language

Resolve vague terms from the accepted model and source where possible. If "account" could still mean Customer or User with different behavior, ask with a recommendation only when that distinction crosses the supplied boundary; otherwise choose the consistent term and record the delegated assumption.

### Discuss concrete scenarios

Stress-test relationships with concrete in-scope scenarios. Resolve cases covered by existing decisions or outside the decision aperture yourself; bring back only scenarios that cross the human frontier or expose a costly commitment.
