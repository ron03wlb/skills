...
4. For a clean bounded Preview, build and validate one `workflow-clean-path-delegation:v1` under `ron-workflow/delegations/`, mode `0600`. It may grant exactly one `publish_bootstrap_spec`.
...
7. Use `grant-derive` to create the exact `workflow-authorization:v1` Issue record. Supply the read-back contract's Issue, type, Spec IDs/hashes, Wiki/Preview binding, and owned-paths hash; every field must match the delegation and requested Grant. Post and read the Grant back before creating a Lane. It binds the Spec and contract plus `wiki_operation: bootstrap`, `wiki_baseline_requirement: missing-with-bootstrap-preview`, the Preview, exact paths, `execute`, and `close_standalone`.
...
9. Invoke `close-issue` in Standalone mode. Only one fixed complete candidate may pass independent Wiki review, local target advancement, target verification, Issue closure, and the atomic config transition to `ready`.

The target remains `missing` throughout bootstrap. Delete the local delegation only after the derived Issue Grant reads back; delete the Preview only after its matching Change Spec reads back. Preserve both on failure.
