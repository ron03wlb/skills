...
4. After the exact Issue is published, use `/pre-execute-issue <Issue-ID>` directly or let the same authorized active execution lane hand off one exact declared prerequisite. It creates or reuses the Issue worktree, invokes `prepare-prerequisite-artifact` when needed, presents the committed Operator SQL for human execution, and records or reuses one content-bound v2 attestation. Direct entry stops after read-back; active entry returns only to the same execution lane after fresh identity and ancestry checks.
