...
Tracker Spec publication routes through the separately installed authorized coordinator, while direct human invocation of the individual `execute-issue` and `close-issue` leaves remains available. The coordinator may use one valid **DAG Run Grant** to invoke those leaves without per-Issue approval. It does not create or broaden leaf authority: each leaf revalidates the Grant and keeps its existing implementation, closeout, tracker, worktree, push, deploy, prerequisite, and scope boundaries. The Codex-only coordinator is neither promoted nor packaged by this shared skill set.
