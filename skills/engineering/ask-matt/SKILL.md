...
3. Follow the published command:
   ...
   - Multi-Issue Tracker Spec → `/to-tickets <Spec-ID>` to consume the completed `to-spec` handoff, settle its Workflow plan checkpoint, publish the Decomposition publication record and composite handoff, then `/run-issue-workflow <Spec-ID>`.
   Fresh `to-spec` publication has no target operational-plan checkpoint or prospective contribution record. `to-tickets` retains its existing Workflow plan checkpoint and invokes model-invoked `/attest-target-contribution` with its exact active producer packet; the helper needs no second confirmation and returns only the immutable record identity. Valid incomplete legacy and `to-spec@v1` operations keep frozen exact resume behavior.
