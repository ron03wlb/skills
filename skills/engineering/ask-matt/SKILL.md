...
The personal `/run-issue-workflow` uses its installed entry and real current-host tools, preserves each Run’s original Grant and retained package while recording compatible current runtime observations, and offers text controls when the panel is unavailable. An explicit comma-separated Spec batch rotates ready work within shared and per-Run worker limits; unchanged authorization and verification evidence is reused. It carries approved pre-Run maintenance through [Run preparation](../../../docs/agents/run-preparation.md#approved-pre-run-workflow-maintenance) without repeated leaf commands; consumer configuration stays static.
