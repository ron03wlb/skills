...
Before interpreting an absent field, inspect the parent or linked Spec's ordered history, including local-file tracker history, for one durable `workflow_artifacts_contract_adopted:v1` record that exactly matches the completion's repository, tracker, Spec, and Issue target branch. When that record precedes the completion note, the note is prospective and without `workflowArtifacts` verification must stop. A completion that definitely precedes adoption, or whose exact scope has no adoption record, remains readable as legacy under its original contract; never retrofit it. Duplicate, malformed, mismatched, or unreadable records plausibly bound to the same repository, tracker, and Spec stop, while a well-formed record for another exact scope does not classify this one.
