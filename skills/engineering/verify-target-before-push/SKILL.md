...
Before interpreting an absent field, inspect the Issue's ordered tracker history, including local-file tracker history, for one immutable `workflow_artifacts_adopted:v1` record that precedes the completion note and exactly matches its Issue, Issue target branch and worktree, topic branch and worktree, Execution baseline, and final candidate. One exact matching record makes that attempt prospective and without `workflowArtifacts` verification must stop. No matching record leaves an older completion note readable as legacy under its original contract; never retrofit it. Duplicate, malformed, mismatched, or unreadable records for the exact attempt stop, while records bound to another exact attempt do not classify this one.
...
The adoption record is compatibility evidence only and grants no implementation, review, coverage, verification, close, push, or deployment authority.
