...
Use the immutable tracker-server creation timestamp against the `workflowArtifacts` completion-note cutover instant `2026-08-31T03:25:20Z` before interpreting an absent field. A note created at or after that UTC instant is prospective and without `workflowArtifacts` verification must stop. Only a note whose server creation timestamp is definitely earlier may remain a legacy completion note without `workflowArtifacts` under its original contract; never retrofit it. A missing, mutable, ambiguous, or unreadable tracker timestamp stops without substituting a body timestamp, candidate commit time, or self-declared schema marker.
