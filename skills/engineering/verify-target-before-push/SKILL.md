...
Use Git ancestry against the `workflowArtifacts` cutover commit `b1fcf9930056b1f1b907e380cd9015bd6162899d` before interpreting an absent field. Run `git merge-base --is-ancestor <cutover> <C>`: when the cutover commit is an ancestor of exact candidate `C`, the note is prospective and without `workflowArtifacts` verification must stop. Otherwise run the reverse ancestry check and accept a legacy completion note without `workflowArtifacts` under its original contract only when candidate `C` is an ancestor of the cutover; never retrofit it. A missing commit object, unreadable result, or divergent history where neither commit is an ancestor of the other stops without inferring legacy status from a timestamp or self-declared marker.
