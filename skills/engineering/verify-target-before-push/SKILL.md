...
Execution completion notes are the sole Issue-to-commit mapping authority. Require each current completion's `operationIdentity` to match its canonical repository, Spec, approved publication identity or hash, and Issue. A field-less completion is legacy only through exact membership in the Spec-scoped `workflow_operation_identity_contract_adopted:v1` `legacyCompletionFrontier` defined in [`references/operation-identity.md`](references/operation-identity.md). Candidate `C` reachable from `V` but not `B` is a member; a baseline-reachable candidate is not. An open unreachable candidate is concurrent work outside this range. A closed unreachable candidate or a reachable open member is contradictory and stops. Freeze the set before review.
