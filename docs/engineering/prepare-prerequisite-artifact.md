...
Type `/prepare-prerequisite-artifact`, or the [agent](https://www.aihero.dev/ai-coding-dictionary/agent) reaches for it automatically when an active prerequisite-preparation handoff fits. Without that exact handoff it stops; this [model](https://www.aihero.dev/ai-coding-dictionary/model)-invoked helper is not a public starting route for prerequisite work.
...
Deterministic validation and independent Standards and [Spec](https://www.aihero.dev/ai-coding-dictionary/spec) review share ten material repair waves. Advisories stay visible without blocking; a confirmed violation or validation failure must be repaired before the candidate is ready.
