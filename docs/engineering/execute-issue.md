...
After the clean candidate passes verification and review, execution appends or reuses one durable `workflow_artifacts_contract_adopted:v1` record in the parent or linked Spec before the first prospective completion. It binds the exact repository, tracker, Spec, and Issue target branch, including for local-file trackers. Every later completion in that scope requires the field; a completion that definitely predates adoption or belongs to an unadopted scope remains legacy. Plausibly bound malformed or duplicate records stop. The record grants no workflow authority.
