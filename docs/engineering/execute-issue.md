...
After the clean candidate passes verification and review, execution appends or reuses one logical `workflow_artifacts_contract_adopted:v1` record in the parent or linked Spec before the first prospective completion. It binds the exact repository, tracker, Spec, and Issue target branch, including for local-file trackers. Payload-identical records from concurrent first completions collapse idempotently at the earliest position; malformed or conflicting records stop. Every later completion in that scope requires the field, while a completion that definitely predates adoption or belongs to an unadopted scope remains legacy. The record grants no workflow authority.
