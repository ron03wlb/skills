...
After the clean candidate passes verification and review, a new attempt appends or reuses and reads back one immutable `workflow_artifacts_adopted:v1` record before its completion note. The record binds the exact Issue, target and topic branch/worktree identities, Execution baseline, and final candidate. That repository/tracker-specific evidence works for hosted and local-file trackers: a matching record makes the immediately following completion prospective and requires the field, while an older completion without one remains legacy. The record grants no workflow authority.
