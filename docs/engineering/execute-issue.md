...
The stable cutover is the immutable tracker-server completion-note creation time `2026-08-31T03:25:20Z`: every note created at or after it must carry the field. Only definitely earlier notes retain their original legacy contract; missing or unreadable server time stops instead of trusting a body timestamp, candidate commit time, or self-declared marker.
