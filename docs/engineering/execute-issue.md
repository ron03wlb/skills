...
It trusts the published delivery classification and records the branch from which the worktree was created as the Issue's only default merge target. Coordinator entry fails closed unless the Grant binds that exact Spec, target, classification, scope, and Multi-Issue decomposition record. The Planning Seal must be reachable and current, while expected paths remain discovery hints rather than an allowlist. If the Issue declares a Manual prerequisite, one human attestation naming the exact executed artifact is sufficient; missing evidence points to [pre-execute-issue](https://aihero.dev/skills-pre-execute-issue) with that path.
