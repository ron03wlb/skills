...
One exception is deliberately narrow: a published documentation-only Seal candidate may reuse an already non-empty Planning Seal only when the Issue names that Seal, its sole parent, and the complete changed-path list, the Seal is this attempt's baseline and candidate, and the declared scope excludes runtime, schema, API, deployment, Manual prerequisites, and all other execution work. Review then examines the Seal's parent-to-Seal diff and completion records the `planning_seal_documentation:v1` review basis; no empty marker commit is permitted. Any missing or mismatched condition returns to the normal non-empty execution candidate rule.
