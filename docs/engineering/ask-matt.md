...
- Multi-Issue Tracker Spec → [to-tickets](https://aihero.dev/skills-to-tickets) to consume the completed `to-spec` handoff, settle its workflow plan checkpoint, publish the Decomposition publication record and composite handoff, then `/run-issue-workflow <Spec-ID>`.
...
Fresh `to-spec` publication creates no target operational-plan checkpoint or prospective contribution record. [to-tickets](https://aihero.dev/skills-to-tickets) retains its workflow plan checkpoint and invokes [model](https://www.aihero.dev/ai-coding-dictionary/model)-invoked [attest-target-contribution](https://aihero.dev/skills-attest-target-contribution) with its exact active producer packet; the helper needs no second confirmation and returns only the immutable record identity. Existing valid incomplete legacy and `to-spec@v1` operations retain frozen exact resume behavior.
