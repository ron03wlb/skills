...
Known permissions and SQL prerequisites belong before Run-ready, through planning and [pre-execute-issue](https://aihero.dev/skills-pre-execute-issue). An explicit comma-separated Spec batch keeps separate Runs and Grants while rotating ready work within a shared worker bound. Healthy waits continue across observation windows; recoverable technical failures transfer exclusive ownership of the original Issue worktree to a separate task for diagnosis and bounded repair. Failed integration keeps the merged candidate, worktree, and completion history until the exact changed inputs pass verification. Governing workflow defects use an independently scoped source maintenance task and verified installation before reconciling the same Run.
