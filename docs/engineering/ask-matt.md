...
The main flow uses [implement](https://aihero.dev/skills-implement) by default. Only when you explicitly select a dedicated-worktree alternative does it replace that step with [execute-issue](https://aihero.dev/skills-execute-issue) followed by [close-issue](https://aihero.dev/skills-close-issue). The independent [wiki](https://aihero.dev/skills-wiki) flow handles Wiki-only edits, and [remove-ron](https://aihero.dev/skills-remove-ron) removes the retired repository-local Ron setup footprint.
