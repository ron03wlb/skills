...
After configuration, setup reads the current harness's resolved installations and reports the configured tracker, triage labels when applicable, required public skill surfaces, operation-scoped producer store, producer handoff, target reader, shared target writer, and Run composition. For hosted trackers it compares every configured triage label with the provider's read-only label listing; the local-file adapter validates its `Status:` mappings because it has no separate label registry. A missing or unknown seam names its owning source, observed evidence, and the smallest human action there, such as repairing tracker access or updating the package that owns the adapter.
