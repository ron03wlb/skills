...
The verifier validates each current owner-derived `operationIdentity` and every declared `workflowArtifacts` path, requirement source, purpose, candidate-diff ownership, and review result. A completion whose candidate equals its execution baseline additionally needs the narrow documentation-only Seal (`planning_seal_documentation:v1`) review basis: its published Issue, non-empty Seal-parent diff, exact documentation paths, explicit empty workflow-artifact list, and excluded runtime, schema, API, deployment, Manual prerequisites, and other execution work must all agree. Independent logical `workflow_operation_identity_contract_adopted:v1` and `workflow_artifacts_contract_adopted:v1` records in the parent or linked Spec bind the exact repository, tracker, Spec, and Issue target branch plus separate frozen `legacyCompletionFrontier` lists of note identities or durable local locators and body digests. Only exact members may omit the corresponding field; neither record classifies the other field. Payload-identical records collapse, while malformed, conflicting, duplicate, unreadable, mismatched, or digest-mismatched evidence stops. A truly unadopted scope remains legacy-compatible. Adoption evidence grants no range membership, coverage, verification, push, or deployment authority.
