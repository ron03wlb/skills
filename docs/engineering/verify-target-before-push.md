...
When present, `workflowArtifacts` names exact required non-contract paths, requirement sources, and purposes inside a member's ordinary Issue contribution. The verifier validates their candidate diff ownership and review evidence. One preceding durable `workflow_artifacts_contract_adopted:v1` record in the parent or linked Spec matching the exact repository, tracker, Spec, and Issue target branch makes every later hosted or local-file completion in that scope prospective and requires the field. A completion that definitely predates adoption or belongs to an unadopted scope remains legacy; plausibly bound malformed or duplicate records stop. Declared artifacts remain visible to aggregate Standards and Spec review, selected-range coverage, focused verification, the full suite, cleanliness, and ref stability. Neither the record nor declaration is an additional coverage source or verification authority.
