...
When present, `workflowArtifacts` names exact required non-contract paths, requirement sources, and purposes inside a member's ordinary Issue contribution. The verifier validates their candidate diff ownership and review evidence. Immutable tracker-server completion-note creation time against cutover instant `2026-08-31T03:25:20Z` is the stable compatibility boundary: notes created at or after it require the field, only definitely earlier notes retain the legacy contract without it, and missing or unreadable server time stops. Declared artifacts remain visible to aggregate Standards and Spec review, selected-range coverage, focused verification, the full suite, cleanliness, and ref stability. The declaration is never an additional coverage source or verification authority.
