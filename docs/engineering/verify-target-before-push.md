...
When present, `workflowArtifacts` names exact required non-contract paths, requirement sources, and purposes inside a member's ordinary Issue contribution. The verifier validates their candidate diff ownership and review evidence. One preceding logical `workflow_artifacts_contract_adopted:v1` record in the parent or linked Spec matching the exact repository, tracker, Spec, and Issue target branch makes every later hosted or local-file completion in that scope prospective and requires the field. Payload-identical concurrent records collapse at the earliest position; malformed or conflicting records stop. A completion that definitely predates adoption or belongs to an unadopted scope remains legacy. Declared artifacts remain visible to aggregate Standards and Spec review, selected-range coverage, focused verification, the full suite, cleanliness, and ref stability. Neither the record nor declaration is an additional coverage source or verification authority.
