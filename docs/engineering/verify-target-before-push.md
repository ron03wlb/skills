...
When present, `workflowArtifacts` names exact required non-contract paths, requirement sources, and purposes inside a member's ordinary Issue contribution. The verifier validates their candidate diff ownership and review evidence. One earlier immutable `workflow_artifacts_adopted:v1` record matching the completion's exact Issue, target and topic branch/worktree identities, Execution baseline, and final candidate makes that hosted or local-file attempt prospective and requires the field. An older completion without a matching record remains legacy under its original contract; ambiguous records stop. Declared artifacts remain visible to aggregate Standards and Spec review, selected-range coverage, focused verification, the full suite, cleanliness, and ref stability. Neither the record nor declaration is an additional coverage source or verification authority.
