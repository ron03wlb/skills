...
A launcher that exits when its child is released is recorded as already exited through its retained process handle. It does not invalidate recovery of the remaining verified helpers. An unreadable live helper still blocks cleanup. If the inspector is interrupted after durable progress, one automatic continuation may release only the original uncompleted identities when the reservation, progress, `UNKNOWN` result and current holders all agree exactly; a durable resume marker prevents any later automatic batch. A mismatch, newcomer or already attempted resume stops for owning-source diagnosis.
