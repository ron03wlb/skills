...
For an authorized coordinator close, the native result is candidate-bound and records three separate owner timestamps: repository close lease acquisition, target writer acquisition, and successful tracker close read-back. The coordinator validates their order before recording delivery progression. These timestamps are evidence about the close owner's stages; they never substitute for current Git, worktree, tracker, or lease checks, and tracker `closed_at` alone cannot manufacture a close-owner completion event.

Failure results stay compact and owner-bound too. The coordinator retains only allowlisted evidence identities and bounded facts, checks referenced integration evidence against its current owner record, and routes malformed, stale, or foreign results to the evidence producer for diagnosis instead of treating or storing them as close progress.

For new completion notes, closeout validates the owner-derived `operationIdentity` plus every `workflowArtifacts` path, requirement source, purpose, contribution ownership, and candidate-bound review result before merging. When candidate and execution baseline are equal, it instead requires the narrow documentation-only Seal (`planning_seal_documentation:v1`) review basis: the published Issue must bind the same non-empty Planning Seal, its sole parent, exact documentation paths, and exclusions for runtime, schema, API, deployment, Manual prerequisites, and every other execution change. Independent logical `workflow_operation_identity_contract_adopted:v1` and `workflow_artifacts_contract_adopted:v1` records in the parent or linked Spec bind the exact repository, tracker, Spec, and Issue target branch plus separate frozen `legacyCompletionFrontier` lists of note identities or durable local locators and body digests. Only exact members may omit the corresponding field; neither record classifies the other field. Payload-identical records collapse, while malformed, conflicting, duplicate, unreadable, mismatched, or digest-mismatched evidence stops. A truly unadopted scope remains legacy-compatible. Adoption evidence grants no candidate, contribution, verification, closeout, push, or deployment authority.
