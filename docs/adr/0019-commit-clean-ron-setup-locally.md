...
If the target checkout is dirty with overlapping work, setup uses an isolated worktree when it can do so without ambiguity. Dirty overlap, unexpected paths, merge conflict, failed tracker capability proof, failed config validation, or any target drift stops and presents the problem and trade-offs. Setup never refreshes its authority to a moved target automatically.
