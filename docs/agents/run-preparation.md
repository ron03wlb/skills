...
Load "grilling" as the named model-invoked owner of decision boundaries, reusing an already loaded matching source. Use the generic Skill tool when available, otherwise the host-supported mechanism to read its required `SKILL.md`, preserving source and invocation restrictions. Apply its SQL boundary: present the proposed effect and obtain missing approval before editing the artifact, including embedded queries. Prepare and validate approved SQL before dependent application implementation; keep independent work eligible. Reuse exact prior approval and ask only about a changed adjustment, effect, environment, or permission. Artifact preparation approval grants no database application authority; declared Manual prerequisites still require the owning preparation and attestation flow below.
