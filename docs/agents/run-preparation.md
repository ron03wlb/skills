...
Load "grilling" as the named model-invoked owner of decision boundaries, reusing an already loaded matching source. Use the generic Skill tool when available, otherwise the host-supported mechanism to read its required `SKILL.md`, preserving source and invocation restrictions. Apply its SQL boundary: table DDL, data corrections, destructive or costly effects, and any migration cost need the missing exact approval; routine reversible application queries and bindings are delegated within existing scope. Present the reserved change set before asking, then prepare and validate approved SQL before dependent application implementation; keep independent work eligible. Reuse exact prior approval and ask only about a changed reserved effect, environment, or permission. Artifact preparation grants no database application authority; declared Manual prerequisites still require the owning preparation and attestation flow below.
